...
class AlexaDriver(BaseDriver):
    ...
    config_schema = [
        {
            ...
            "type": "number",
            "required": True,
            "description": "The HomeHub Alexa integration account that owns this Echo device.",
        },
        {
            "name": "serial_number",
            "label": "Alexa device serial",
            "type": "string",
            "required": True,
            "description": "Usually filled automatically when the device is discovered from your Alexa account.",
        },
        {"name": "spotify_account_id", "label": "Spotify account", "type": "number", "required": False},
    ...
    setup_schema = {
        "description": "Link this Echo to a configured Amazon Alexa account, then verify that HomeHub can see the device.",
        "requires_ip": False,
        "instructions": [
            "Alexa devices are controlled through the configured Amazon account rather than directly by IP address.",
            "Choose an Alexa integration account. If you do not have one yet, configure it in this wizard using your Amazon account details.",
            "For discovered Echo devices the serial number is filled automatically. For manual setup, enter the serial shown by Alexa discovery/account data.",
            "The Alexa integration uses an unofficial web API and Amazon may occasionally require re-authentication.",
        ],
        "account_provider": "alexa",
        ...
        "optional_accounts": [
            {
                ...
                "description": "Optional — enables Spotify search and playback controls.",
            }
        ],
    ...
    async def _api(self):
        ...
        device = next(
            (
                item
                for item in devices
                if item.get("serialNumber") == serial or item.get("serial_number") == serial
            ),
            None,
        )
        if not device:
    ...
    async def action_volume(self, value):
        return await self._call("set_volume", max(0, min(100, int(value))) / 100)

    async def action_announcement(self, text):
        return await self._call("send_announcement", text)

    async def action_tts(self, text):
        return await self._call("send_tts", text)

    async def action_custom(self, text):
        return await self._call("run_custom", text)
